...
public abstract class LevelParent {
    ...
    /**
     * Spawns bomber enemies within a certain vertical range based on a given probability.
     *
     * @param spawnProbability The probability of spawning each bomber enemy.
     */
    protected void spawnBomberEnemies(double spawnProbability) {
        double positionMultiplier = 0.25;
        if (Math.random() < spawnProbability) {
            double newEnemyBomberInitialYPosition = Math.random() * getEnemyMaximumYPosition() * positionMultiplier;
            ActiveActor newEnemy = new BomberPlane(getScreenWidth(), newEnemyBomberInitialYPosition);
            addEnemyUnit(newEnemy);
        }
    }

    /**
     * Spawns tanker enemies based on a given probability.
     *
     * @param spawnProbability The probability of spawning each tanker enemy.
     */
